...
async function connectToDatabase() {
  try {
    const uri = process.env.MONGODB_URI || 'mongodb://localhost:27017/chatdb';
...
async function saveMessageToDb(message) {
  try {
    if (db) {
      ...
      await db.collection('messages').insertOne(messageWithTimestamp);
    }
  } catch (error) {
  ...
  try {
    if (db) {
      ...
      if (allMessages.length > 1000) {
        // 删除最旧的消息，只保留最新的1000条
        const messagesToDelete = allMessages.slice(0, allMessages.length - 1000);
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (data) => {
    try {
      ...
      // 保存消息到数据库 - 持久化存储
      try {
        await saveMessageToDb(broadcastMessage);
        // 清理旧消息以控制数据库大小
